  ...
  <div class="card-body">
    ...
    <div class="row g-3 align-items-end">
      ...
      <div class="col-md-3">
        ...
        <select class="form-select" name="minggu_ke" id="mingguKeSelect" disabled>
          @for($w = 1; $w <= $totalWeeks; $w++)
            <option value="{{ $w }}" {{ $w === ($mingguKe ?? $progress->minggu_ke) ? 'selected' : '' }}>Minggu {{ $w }}</option>
          @endfor
        </select>
        <input type="hidden" name="minggu_ke" value="{{ $mingguKe ?? $progress->minggu_ke }}">
      </div>
      <div class="col-md-3">
        <label class="form-label">Tanggal</label>
        <input type="date" class="form-control" name="tanggal" id="tanggalInput" value="{{ $tanggal ?? \Carbon\Carbon::parse($progress->tanggal)->toDateString() }}">
...
(function(){
  ...
  // Initial update (no event listener karena dropdown disabled)
  updateDateRange();
  @endif
