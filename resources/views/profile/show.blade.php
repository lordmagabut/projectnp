      ...
      <div class="card-body">
        @auth
          <dl class="row mb-0">
            ...
            <dd class="col-7">{{ $hostname && $hostname !== $ip ? $hostname : '—' }}</dd>
    ...
    <div class="card shadow-sm">
      <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <form method="get" class="d-flex gap-2">
          <input type="text" class="form-control form-control-sm" name="q" value="{{ request('q') }}" placeholder="Cari event/desc/ip...">
            ...
            <tbody>
              @forelse($logs as $log)
                <tr>
                  ...
                  <td><span class="badge {{ $log->event === 'login' ? 'bg-success' : ($log->event === 'logout' ? 'bg-secondary' : 'bg-info') }}">
                    {{ $log->event }}
                  </span></td>
